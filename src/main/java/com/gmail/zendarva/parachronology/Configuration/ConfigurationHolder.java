...
//		        }
    ...
    private void setDisplacementsDefaults(Configuration con) {
        ...
        if (Loader.isModLoaded("fp")) {
            defaultTier2Sand.add("fp:neon_sand:0");
            defaultTier2Sand.add("fp:neon_sand:4");

            ArrayList fpTier2IronBlock = new ArrayList<String>(
                    Arrays.asList("fp:color_iron:0", "fp:color_iron:8"));
            prop = con.get("Displacements.Tier2", "minecraft:iron_block", (String[]) fpTier2IronBlock.toArray(new String[fpTier2IronBlock.size()]));

            prop.set((String[]) fpTier2IronBlock.toArray(new String[fpTier2IronBlock.size()]));
            DisplaceListBuilder.Instance().addDisplacement(2, BlockReference.readBlockFromString("minecraft:iron_block"),
                    prop.getStringList());
        }

        if (Loader.isModLoaded("forestry")) {

            ArrayList forestryTier2HayBlock = new ArrayList<String>(
                    Arrays.asList("forestry:beehives:0","forestry:beehives:1","forestry:beehives:2","forestry:beehives:3","forestry:beehives:4",
                            "forestry:beehives:5","forestry:beehives:6"));
            prop = con.get("Displacements.Tier2", "minecraft:hay_block", (String[]) forestryTier2HayBlock.toArray(new String[forestryTier2HayBlock.size()]));

            prop.set((String[]) forestryTier2HayBlock.toArray(new String[forestryTier2HayBlock.size()]));
            DisplaceListBuilder.Instance().addDisplacement(2, BlockReference.readBlockFromString("minecraft:hay_block"),
                    prop.getStringList());
        }

        if (Loader.isModLoaded("extrabees")) {

            ArrayList binnieTier2Hive = new ArrayList<String>(
                    Arrays.asList("extrabees:hive:0","extrabees:hive:1","extrabees:hive:2","extrabees:hive:3"));
            prop = con.get("Displacements.Tier2", "forestry:beehives:-1", (String[]) binnieTier2Hive.toArray(new String[binnieTier2Hive.size()]));

            prop.set((String[]) binnieTier2Hive.toArray(new String[binnieTier2Hive.size()]));
            DisplaceListBuilder.Instance().addDisplacement(2, BlockReference.readBlockFromString("forestry:beehive:-1"),
                    prop.getStringList());
        }
    ...
    private void buildDefaults() {
        ...
        if (Loader.isModLoaded("integrateddynamics")){
            defaultTier1Sapling.add("integrateddynamics:menril_sapling");
        }
        if (Loader.isModLoaded("mekanism")){
            defaultTier2Stone.add("mekanism:oreblock");
        }

        if (Loader.isModLoaded("deepresonance")){
            defaultTier3Stone.add("deepresonance:resonating_ore");
        }
        if (Loader.isModLoaded("fp")) {
            defaultTier2Sand.add("fp:neon_sand:0");
            defaultTier2Sand.add("fp:neon_sand:4");
        ...
        if (Loader.isModLoaded("forestry")) {
            defaultTier2Stone.add("forestry:resources");
        }
