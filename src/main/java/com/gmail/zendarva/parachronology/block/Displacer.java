...
public class Displacer extends Block implements ITileEntityProvider {
    ...
    public Displacer(Material p_i45394_1_) {
        ...
        GameRegistry.register(new DiplacerItemBlock(this));
    }
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        ...
        bb = new AxisAlignedBB(pos.getX() - 3, pos.getY() - 3, pos.getZ() - 3, pos.getX() + 3, pos.getY() + 3, pos.getZ() + 3);
        ...
        for (EntityItem item : items) {
            ...
            {
                ItemStack petrified = new ItemStack(Parachronology.petrifiedWood,item.getEntityItem().getCount());
        ...
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                for (int z = -1; z <= 1; z++) {
                    ...
                    BlockReference ref = BlockReference.readBlockFromString(Block.REGISTRY.getNameForObject(block) + ":" + block.getMetaFromState(world.getBlockState(tempPos)));
