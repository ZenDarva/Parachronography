...
public class Moment extends Item {
    ...
    private EnumActionResult transformUse(EntityPlayer player, World world, ItemStack stack, BlockPos targ, int amount) {
        Random r  = new Random();
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase entity, EnumHand hand) {
        ...
        if (transforms.size() == 0)
            return false;
        for (int i = 0;i<25;i++) {
            entity.world.spawnParticle(EnumParticleTypes.PORTAL, entity.posX -.5 + r.nextFloat(), entity.posY, entity.posZ -.5 + +r.nextFloat(), r.nextFloat() *.5, r.nextFloat()*.5, r.nextFloat()*.5);
        }
        if (player.world.isRemote)
            return true;
        stack.shrink(1);
